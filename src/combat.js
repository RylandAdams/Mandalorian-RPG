...
export default class Combat {
	...
	playerAttackFirst(player, enemy) {
		...
		switch (weaponSelect) {
			case '1':
				if (
					player.strength + (Math.floor(Math.random() * 20) + 1) >
					enemy.armorTotal
				) {
					let damageDealt =
						player.equippedMeleeWeapon.weaponStrength +
						(Math.floor(Math.random() * 20) + 1);
					enemy.health -= damageDealt;
					console.log(
						`You have dealt the ${enemy.id} ${damageDealt} damage! They appear to have ${enemy.health} health remaining.`
					);
				} else {
			...
			case '2':
				if (
					player.precision + (Math.floor(Math.random() * 20) + 1) >
					enemy.armorTotal
				) {
					console.log('You Hit them!');
					enemy.health -=
						player.equippedRangedWeapon.weaponStrength +
						(Math.floor(Math.random() * 20) + 1);
				} else {
					console.log('You Missed.');
				}
			...
			case '3':
				if (
					player.intellect + (Math.floor(Math.random() * 20) + 1) >
					enemy.armorTotal
				) {
					console.log('You Hit them!');
					enemy.health -=
						player.equippedAbility.weaponStrength +
						(Math.floor(Math.random() * 20) + 1);
				} else {
					console.log('You Missed.');
				}
		...
		if (
			enemy.weaponStrength + (Math.floor(Math.random() * 20) + 1) >
			player.armorTotal
		) {
